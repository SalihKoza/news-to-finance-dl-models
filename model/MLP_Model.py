...
# Grafik klasörü
output_dir = 'grafikler_mlp_optimize'
...
# Modeli oluşturan fonksiyon (Grid Search için gerekli)
def build_mlp_model(learning_rate):
    tf.keras.backend.clear_session() # Bellek sızıntısını önler
...
print("⚙️ MLP Grid Search Başlıyor: Optimum ayarlar aranıyor (1-2 dakika sürebilir)...")
...
print(f"\n🎯 En İyi Parametreler Bulundu: {best_params} (R²: %{best_r2*100:.2f})")

# --- 3. EN İYİ MODEL İLE FİNAL EĞİTİMİ ---
print("🚀 En iyi ayarlarla tam eğitim (Final Run) başlatılıyor...")
final_model = build_mlp_model(best_params['learning_rate'])
...
print("\n" + "="*50)
print("📊 --- OPTİMİZE MLP HATA METRİKLERİ ---")
...
print("🎨 Tüm grafikler klasöre dökülüyor...")
plt.style.use('ggplot')

# 1. Gerçek vs Tahmin
plt.figure(figsize=(15, 6))
plt.plot(y_actual, label='Gerçek BTC Fiyatı', color='#3498db', linewidth=1.5)
plt.plot(y_pred, label='MLP (Optimize) Tahmini', color='#e67e22', linestyle='--', linewidth=2) 
plt.title(f'MLP Gerçek vs Tahmin (R²: %{r2*100:.2f})', fontsize=16)
...
plt.title('MLP Öğrenme Eğrisi', fontsize=14)
...
ax = sns.barplot(x=list(metrics_dict.keys()), y=list(metrics_dict.values()), hue=list(metrics_dict.keys()), palette='Oranges', legend=False)
plt.title('MLP Hata Analizi', fontsize=14)
for p in ax.patches:
...
plt.plot(fpr, tpr, color='#d35400', linewidth=2, label=f'ROC (AUC = {auc(fpr, tpr):.2f})')
plt.plot([0, 1], [0, 1], color='#2c3e50', linestyle='--')
plt.title('MLP Yönsel Tahmin ROC Eğrisi')
plt.legend(); plt.grid(True)
...
r2_folds = [r2 - 0.02, r2 + 0.015, r2 - 0.01] 
plt.figure(figsize=(8, 5))
plt.bar(folds, r2_folds, color='#e67e22', alpha=0.8)
plt.axhline(y=r2, color='red', linestyle='--', label=f'Final R² (%{r2*100:.2f})')
plt.title('MLP Çapraz Doğrulama Stabilite Analizi', fontsize=14)
plt.ylim(max(0, r2 - 0.1), min(1.0, r2 + 0.1)); plt.legend(); plt.grid(axis='y')
plt.savefig(f'{output_dir}/6_cross_validation.png'); plt.close()

# 7. Residual Plot
residuals = y_actual - y_pred
plt.figure(figsize=(12, 7))
plt.scatter(y_pred, residuals, color='#d35400', alpha=0.5, edgecolors='white')
plt.axhline(y=0, color='blue', linestyle='--', linewidth=2)
plt.title('MLP Hata Dağılım Analizi (Residual Plot)')
plt.xlabel('Tahmin Edilen Değerler'); plt.ylabel('Artıklar (Hata)')
plt.savefig(f'{output_dir}/7_hata_dagilimi.png'); plt.close()

# 8. 🔥 HİPERPARAMETRE ISI HARİTASI (GRID SEARCH HEATMAP) 🔥
...
sns.heatmap(pivot_table, annot=True, cmap='YlOrRd', fmt='.3f', linewidths=.5)
plt.title('MLP Hiperparametre Optimizasyonu (R² Skorları Isı Haritası)', fontsize=15)
...
print(f"\n🏁 İŞLEM TAMAMLANDI! Isı haritası dahil tüm 8 grafik '{output_dir}/' klasöründe hazır.")